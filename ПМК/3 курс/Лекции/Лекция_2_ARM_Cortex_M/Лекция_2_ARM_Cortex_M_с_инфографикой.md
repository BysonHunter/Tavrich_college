...
> Материал построен по исходной презентации. Там, где это помогает пониманию архитектуры, добавлены пояснения и небольшие практические примеры. Текстовые схемы и упрощённые рисунки заменены на более наглядные инфографики, остальные иллюстрации сохранены.
...
![ARM, ядро Cortex-M и производители микроконтроллеров](images/diagram_arm_licensing.png)
...
![История развития ARM и Cortex-M](images/diagram_arm_history.png)
...
![Семейство Cortex-M и области применения](images/diagram_cortex_m_family.png)
...
Одно из преимуществ Cortex-M — сходная логика программирования разных ядер семейства. Это упрощает перенос проектов между микроконтроллерами одного архитектурного семейства.
...
![RISC-архитектура и конвейер Cortex-M](images/diagram_risc_pipeline.png)
...
![Внутреннее устройство ядра Cortex-M3/M4](images/diagram_core_structure.png)
...
![Как NVIC обрабатывает прерывания](images/diagram_nvic.png)
...
На схеме выше показана последовательность обработки прерывания: **событие → IRQ → NVIC → сохранение контекста → переход к обработчику ISR → возврат**.
...
![Системный таймер SysTick](images/diagram_systick.png)
...
![Карта памяти Cortex-M](images/diagram_memory_map.png)
